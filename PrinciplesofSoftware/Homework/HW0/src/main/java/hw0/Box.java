...
public class Box implements Iterable<Ball> {
    ...
    private ArrayList<Ball> sorted_box;
    /**
    ...
    public Box(double maxVolume) {
        ...
        this.max_vol = Double.parseDouble(String.valueOf(maxVolume));
    }
    ...
    public boolean add(Ball b) {
        //  Check to see if it can be added, if then then add it
        if (((b.getVolume() + this.ballContainer.getVolume()) <= this.max_vol)){
    ...
    public Iterator<Ball> getBallsFromSmallest() {
         class VolComparator implements Comparator<Ball>{
            @Override
            public int compare( Ball b1, Ball b2){
                int vol1 = (int)b1.getVolume();
                int vol2 = (int)b2.getVolume();

                return vol1 - vol2;
            }

        }
        this.sorted_box = new ArrayList<Ball>();
        Iterator<Ball> itr = ballContainer.iterator();
        while (itr.hasNext()){
            sorted_box.add(itr.next());
        }
        Collections.sort(sorted_box, new VolComparator());

        return sorted_box.iterator();
    }
