...
public class BallContainer implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        ...
        boolean added = this.contents.add(b);
        if (added == true) {
            this.total_volume += b.getVolume();
        }
        return added;
    }
    ...
    public int differentColors() {
        // Thought about making separate variable and modifying add/remove but remove wouldn't
        // account for duplicate colors

        Set<Color> different_colors = new HashSet<>();
        Iterator<Ball> ball_itr = this.iterator();
        while (ball_itr.hasNext()){
            Color current_color = ball_itr.next().getColor();
            different_colors.add(current_color);
        }
    ...
    public boolean areSameColor() {
        ...
        if (different_colors.size() == 1 || contents.size() == 0){
            return true;
        }else{
            return false;
        }
