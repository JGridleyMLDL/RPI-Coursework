...
void StudentTests(){
    //Concat with empty Rope Test -> shouldn't change
    Rope r1(ConcatTestRopeLeft());
    Rope r2;
    Rope r3(r1);
    std::string s1 = "Hello my name is Simon";
    r1.concat(r2);
    
    std::cout<<"Testing Empty Concat:"<<std::endl;
    std::cout<<"Printing Original: "<<std::endl;
    r3.print(std::cout);
    std::cout<<"Printing r1 with 0 concat: "<<std::endl;
    r1.print(std::cout);
    std::cout<<std::endl;
    
    //Report with same letters Test
    Rope r4(IndexTestRope());
    std::string rs1;
    assert(r1.report(0,0,rs1));         //Return true if single letter is found.
    
    //Invalid Index
    Rope r5(IndexTestRope());
    char c;
    bool i = r5.index(27, c);           //Return false if the index is invalid.
    if(!i){std::cout<<"Invalid Index passed"<<std::endl<<std::endl;}
    
    //Copy/Assignment with empty Rope Test
    Rope r6(IndexTestRope());
    Rope r7;
    Rope r8(r7);            //Copy constructor with empty rope
    r6 = r7;                //Assignment with empty rope
    
    std::cout<<"Printing Copy/Assignment with empty Rope: "<<std::endl;
    r8.print(std::cout);
    r6.print(std::cout);
    std::cout<<std::endl;
    ...
    Rope r9_rhs;
    r9.split(22, r9_rhs);           //Should not change original
    std::cout<<"Printing the split strings (Split at 22): "<<std::endl;
    r9.print_strings(std::cout);
    r9_rhs.print_strings(std::cout);
    std::cout<<std::endl;
    
    //Split at last character
    r9.split(21, r9_rhs);           //Should break off last character
    std::cout<<"Printing the split strings (Split at 21): "<<std::endl;
    r9.print_strings(std::cout);
    r9_rhs.print_strings(std::cout);
    std::cout<<std::endl;
    ...
    std::string s;
    r10.report(11, 30, s);          //Report goes out of string bounds
    assert(s=="");
