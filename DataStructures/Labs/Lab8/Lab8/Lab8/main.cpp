...
bool inVector(int x, int y, vector<Point> const& path){
...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y, vector<Point> const& path_grid, bool solution) {
    if (solution == false){
        ...
        for (int y=0; y<blocked_grid[0].size(); ++y) {
            for (int x=0; x<blocked_grid.size(); ++x) {
                if (x == start_x && y == start_y)
                    ...
                    cout << " X";
                else if (solution == true and inVector(x, y, path_grid)){
                    cout << " $";
                }
                else
                    cout << " .";
...
bool paths(int x, int y, vector<vector<bool> >& grid, vector<Point>& out){
    ...
    if(grid[x][y] == true){
        ...
        good_path = paths(x, y-1, grid, out);
        if(good_path == true){
            out.push_back(Point(x, y));
        }
        return good_path;
        ...
        good_path = paths(x-1, y, grid, out);
        if(good_path == true){out.push_back(Point(x, y));}
        return good_path;
    }
    else{
        good_path = paths(x-1, y, grid, out) || paths(x, y-1, grid, out);
        if(good_path){
            out.push_back(Point(x, y));
        }
        return good_path;
...
int main(int argc, char* argv[]) {
    ...
    vector<Point> path_grid;
    bool solution = true;
    int start_x, start_y;
    read_grid(istr, blocked_grid, start_x, start_y);
    print_grid(blocked_grid, start_x, start_y, path_grid, solution);
    
    

    // Start here with your code...
    int path_num;
    solution = paths(start_x, start_y, blocked_grid, path_grid);
    print_grid(blocked_grid, start_x, start_y, path_grid, solution);
