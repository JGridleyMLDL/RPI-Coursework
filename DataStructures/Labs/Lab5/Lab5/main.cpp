...
template <typename T> int remove_matching_elements(Vec<T> &vector, const T& element){
    //Looping through the vector and checking how many matches.
    int matches = 0;
    T* point;
    unsigned int i = 0;
    while(i<vector.size()){
...
int main() {
  ...
  // ADD MORE TEST CASES HERE
    cout<<"Now testing remove_matching_elements. Testing on v1 with 100"<<endl;
    int removed = remove_matching_elements(v1, 100);
    for ( i = 0; i<v1.size(); ++i )
      cout << " " << v1[i];
    cout << endl;
    cout<<"Elements Removed: "<<removed<<endl<<endl;
