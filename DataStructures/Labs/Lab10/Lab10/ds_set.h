...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
      if(old_root == NULL){return NULL;}                //Base Case
      
      TreeNode<T>* new_copy = new TreeNode<T>;
      new_copy->value = old_root->value;
      
      new_copy->left = copy_tree(old_root->left);
      new_copy->right = copy_tree(old_root->right);
  ...
  iterator find(const T& key_value, TreeNode<T>* p) {
      ...
      while(p->value != key_value && (p->left !=NULL && p->right!=NULL)){
    ...
    iterator findR(const T& key_value, TreeNode<T>* p) {
    if (!p) return iterator(NULL);
    if (p->value > key_value)
