...
class MyApp(object):
    ...
    def draw_lines(self, centerx, centery, length):
        self.canvas.create_line(centerx, centery+length, \
                                centerx, centery-length, fill="black")  #Draws the vertical part of the line
        self.canvas.create_line(centerx+length, centery, \
                                centerx-length, centery, fill="black") #Draws the horizonatal part of the line
        while length > 30:
            return self.draw_lines(centerx-length, centery, length/2),\
